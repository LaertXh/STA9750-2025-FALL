...
knitr:
  opts_chunk:
    results: "hold" # hold output until end of chunk (no per-line splits)
---
...
"Money Heist" takes the lead for a country's tv show that persisted in the top 10. This how was able to stay for 127 weeks in the top 10 category for the country of Pakistan. never seen it, heard its very good. I am sure Pakistan would agree. 
```{r}
COUNTRY_TOP_10 |>
...
Many films do not start of as number one. It takes time for people to give it a real chance. We can see that a total of 45 movies have started as a different rank in their debut week. However, eventually after some time they have made it to number one. The most recent tile to achieve this is "Unknown Number: The High School Catfish"
```{r}
library(dplyr)
library(stringr)
...
Stranger Things has had incredible success in its run. It has accumulated about 2,967,980,000 hours watched, thats right nearly 3 billion, spanning a 19 week run in the top 10 leaderboard. As stated earlier, with numbers like this we can draw other conclusions. Given that the show is generally catered towards a younger audience, its safe to say that Netflix as accumulated a huge customer base in this demographic. 
...
# Yearly hours watched (plus weeks counted)
...
# Optional: add YoY absolute and percent change for context
st_yearly_yoy <- st_yearly |>
  mutate(
    yoy_change = hours_watched - dplyr::lag(hours_watched),
  )

# Nicely formatted table
st_yearly_yoy |>
  datatable(options = list(searching = FALSE, paging = FALSE, info = FALSE)) |>
  formatRound(c("hours_watched", "yoy_change"), digits = 0)
